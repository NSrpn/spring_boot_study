...
(function ($) {
    ...
    function getResponse(response) {
        return response.responseJSON === undefined ? JSON.parse(response.responseText) : response.responseJSON;
    }
    ...
    $(function () {
                                    ...
                                    complete: function (result) {
                                        if (result.status === 200) {
                                            ...
                                            $Middle.find('.Middle-searchHeader').text('Найдено ' +
                                                resp.count + ' книг' +
                                                (resp.count % 10 < 10 && resp.count % 10 > 20 ? (resp.count % 10 === 1 ? 'a' : (resp.count % 10 > 1 && resp.count % 10 < 5 ? 'и' : '')) : ''))
        ...
        Authors().init();
        var Card = function () {
            //todo parametrize image path
            var bookTemplate = function (book) {
                return '<div class="Card">' + '   <div class="Card-picture">' + '<a href="' + book.slug + '">' + '       <img src="/assets/img/content/books/' + book.image + '">\n' + '</a>' +
                                ...
                                if ($(slick.$slides[slick.$slides.length - 3]).hasClass('slick-active')) {
                                    if ($(slick.$slider).data('load')) {
                                        ...
                                        function sendData(address) {
                                            getData(address, {
                                                offset: function() {
                                                    var os = $(slick.$slider).data('loadoffset');
                                                    $(slick.$slider).data('loadoffset', ++os)
                                        ...
                                        switch ($(slick.$slider).data('load')) {
                                            case'recommended':
                                                sendData('/books/slider/recommended')
                                                break;
                                            case'recent':
                                                sendData('/books/slider/recent')
                                                break;
                                            case'popular':
                                                sendData('/books/slider/popular')
                                                break;
                                        }
                                    }
                                }
                ...
                $.ajax({
                    ...
                    complete: function (result) {
                        if (result.status === 200) {
                            var data = '';
                            var resp = getResponse(result);
                            resp.transactions.forEach(function (transaction) {
